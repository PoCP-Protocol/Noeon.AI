...
const { recallFromMemory, applyRecallBoosts } = require('./memory-recall');
const { executeDeclaredSpawns } = require('./spawn-runner');
...
function runFieldEngine(next, options = {}) {
  ...
  let memoryRecall = { signals: [], boosted: [] };
  ...
  if (options.field_memory) {
    ...
    const recall = recallFromMemory(options.field_memory, fieldCfg);
    env.signals = [...new Set([...(env.signals || []), ...recall.signals])];
    const boosted = applyRecallBoosts(initialCells, recall);
    initialCells = boosted.cells;
    memoryRecall = { ...recall, boosted: boosted.boosted };
  }
  ...
  const dominantPreSpawn = [...cells].sort((a, b) => b.energy - a.energy)[0] || null;
  const declaredSpawns = executeDeclaredSpawns(next.spawns || [], cells, {
    dominant: dominantPreSpawn,
    woven,
    narrative
  }, next.autobond);
  cells = mergeSpawnCells(cells, declaredSpawns);
